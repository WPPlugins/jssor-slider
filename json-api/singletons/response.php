...
class WJSSL_JSON_API_Response {

  function setup() {
    global $wjssl_json_api;
    $this->include_values = array();
    $this->exclude_values = array();
    if ($wjssl_json_api->query->include) {
      $this->include_values = explode(',', $wjssl_json_api->query->include);
    }
    // Props to ikesyo for submitting a fix!
    if ($wjssl_json_api->query->exclude) {
      $this->exclude_values = explode(',', $wjssl_json_api->query->exclude);
  ...
  function get_json($data, $status = 'ok') {
    ...
    if ($wjssl_json_api->query->json_encode_options) {
        $json_encode_options = $wjssl_json_api->query->json_encode_options;
    }
    $json = wp_json_encode($data, $json_encode_options);

    // Thanks to Stack Overflow user Gumbo stackoverflow.com/questions/2934563
    if ($wjssl_json_api->query->json_unescaped_unicode) {
  ...
  function respond($result, $status = 'ok', $http_status = 200) {
    ...
    if ($wjssl_json_api->query->dev || !empty($_REQUEST['dev'])) {
    ...
    } else if ($wjssl_json_api->query->redirect) {
      $url = $this->add_status_query_var($wjssl_json_api->query->redirect, $status);
      wp_redirect($url);
    } else if ($wjssl_json_api->query->callback) {
      // Run a JSONP-style callback with the result
      $this->callback($wjssl_json_api->query->callback, $json);
    } else {
      // Output the result
  ...
  function output($result, $http_status = 200) {
    $http_status = apply_filters('wjssl_json_api_http_status', $http_status);
